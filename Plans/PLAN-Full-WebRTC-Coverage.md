...
- [x] Simulcast (`sendEncodings`) — shipped 2026-04-24 in RTC 1.1.5 via new `IRTCPeerConnection.AddTransceiver(string, RTCRtpTransceiverInit)` overload. `RTCRtpTransceiverInit` exposes `SendEncodings` (reusing `RTCRtpEncoding[]`). Browser round-trips to native `addTransceiver({ sendEncodings })`; desktop ignores (SipSorcery has no simulcast). Test `RtpSender_InitSimulcast_SdpOfferContainsSimulcastAndRidLines` parses the SDP offer and confirms `a=simulcast:send` + `a=rid:h send` / `a=rid:m send` / `a=rid:l send` on the wire.
